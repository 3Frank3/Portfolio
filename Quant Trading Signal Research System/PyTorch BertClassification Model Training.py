...
train_df, val_df = train_test_split(df, test_size = 0.3, stratify = df["summary"], random_state = 42)

train_text = train_df["label"].to_list()
val_text = val_df["label"].to_list()

train_topic = train_df["sunnary"].to_list()
val_topic = val_df["summary"].to_list()
